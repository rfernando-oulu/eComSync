...
<tr style={{ backgroundColor: '#f8f8f8'}}>
    ...
    <td>
      <select multiple={true} onChange={(event) => {
        const selectedOptions = Array.from(event.target.selectedOptions, option => option.dataset.id);
        setSelectedOptions(selectedOptions);
      }}>
        {options.map((option) => (
          <option key={option.id} value={option.name} data-id={option.id}>{option.name}</option>
        ))}
      </select>
    </td>
